...
Quando("{string} e {string} solicita a locação deste equipamento") do |email_locatario, pass_locatario|
  user_id_locatario = SessionsService.new.get_user_id(email_locatario, pass_locatario)
  EquiposService.new.booking(@equipo_id, user_id_locatario)
end

Então("devo ver a seguinte mensagem:") do |doc_string|
  #gsub: substring do ruby
